...
"#,
    ...
    fn test_is_duplicate_matching() {
        let date = rustledger_core::rustledger_core::naive_date(2024, 1, 15).unwrap();
        let new_txn =
    ...
    fn test_is_duplicate_different_date() {
        let new_txn = Transaction::new(
            rustledger_core::rustledger_core::naive_date(2024, 1, 15).unwrap(),
            "GROCERY STORE",
        )
        ...
        let existing = vec![
            Transaction::new(
                rustledger_core::rustledger_core::naive_date(2024, 1, 16).unwrap(),
                "GROCERY STORE",
            )
            .with_posting(rustledger_core::Posting::new(
                "Assets:Bank",
